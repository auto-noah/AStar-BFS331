...
class Graph:
    ...
    def bfs(self, begin_id: str, end_id: str) -> Tuple[List[str], float]:
        ...
        *** MUST CALL build_path! ***
        ...
        cur = self.get_vertex_by_id(begin_id)
        end = self.get_vertex_by_id(end_id)

        # If start and end are both valid
        if cur and end:

            # Make the underlying BFS queue and enqueue the starting vertex
            bfs_queue = queue.Queue()
            bfs_queue.put(cur.id)

            # Make a dict to store the back edges (previously visited vertices)
            back_edges = dict()

            # While we still have vertices left to visit
            while bfs_queue.qsize() != 0:

                # Get next vertex and remove it from queue
                cur_id = bfs_queue.get()
                cur = self.get_vertex_by_id(cur_id)

                # If we've reached the end vertex, return the built path tuple
                if cur is end:
                    return self.build_path(back_edges, begin_id, end_id)

                # Add all of cur's adjacent vertices to the underlying queue
                for adj_id in cur.adj:

                    # Grab adjacent vertex
                    adj = self.get_vertex_by_id(adj_id)

                    # If we haven't visited the adjacent vertex before
                    if not adj.visited:

                        # Add it to the underlying queue and back edges
                        # dict as {key->vertex_id : value->cur_id}
                        bfs_queue.put(adj_id)
                        back_edges[adj_id] = cur_id

                    # Check if we've found a path to the end vertex
                    if end_id in back_edges:
                        return self.build_path(back_edges, begin_id, end_id)

                    # Set visited flag for the current vertex before moving on
                    cur.visited = True

        # Return empty if desired start or end vertices aren't contained in the graph
        return [], 0
    ...
    def a_star(self, begin_id: str, end_id: str,
        ...
        # Get start and end vertices
        begin = self.get_vertex_by_id(begin_id)
        end = self.get_vertex_by_id(end_id)

        # If start and end are both valid
        if begin and end:

            # Initialize all distances (except for beginning vertex
            # which = 0) to infinity and add them to the pqueue
            distance = dict()
            pq = PriorityQueue()
            for vertex_id in self.vertices:
                vertex = self.get_vertex_by_id(vertex_id)
                if vertex_id == begin_id:
                    distance[vertex] = 0
                else:
                    distance[vertex] = float('inf')
                pq.push(distance[vertex], vertex)

            # While we have vertices left
            back_edges = dict()
            while not pq.empty():

                # Pop the highest priority vertex
                dis_val, vertex = pq.pop()

                # If we've reached the end and the path is valid
                if vertex is end and distance[end] < float('inf'):
                    return self.build_path(back_edges, begin_id, end_id)

                # For every adjacent vertex
                for adj_id in vertex.adj:
                    adj = self.get_vertex_by_id(adj_id)

                    # Check if the distance is smaller than the previous, updating
                    # (and adding the heuristic for the pq) if it is
                    if distance[vertex] + self.get_edge_by_ids(vertex.id, adj.id)[2] < distance[adj]:
                        distance[adj] = distance[vertex] + self.get_edge_by_ids(vertex.id, adj.id)[2]
                        pq.update(distance[vertex] + self.get_edge_by_ids(vertex.id, adj.id)[2] + metric(adj, end), adj)
                        back_edges[adj.id] = vertex.id

        # Return empty if begin or end ids are invalid
        return [], 0
...
def teleport(system_dict, start, end):
    """
    Finds the shortest path through multiple connected "galaxys" with
    teleportation vertices that are the associations between galaxies.
    :param system_dict: A structured dictionary containing the galaxy information as
                        well as the arrival, departure, and departure destinations.
    :param start: List of string containing exactly two elements, name of start galaxy,
                  and name of start vertex inside the start galaxy, respectively.
    :param end: List of string containing exactly two elements, name of end galaxy, and
                name of end vertex inside the end galaxy, respectively.
    :return: A float value representing distance from starting vertex to ending vertex
    """
    # Grab start and end info
    start_galaxy, start_v_id = start[0], start[1]
    end_galaxy, end_v_id = end[0], end[1]

    if start_galaxy not in system_dict or end_galaxy not in system_dict:
        return float('inf')

    # Return infinity if not valid
    if not system_dict:
        return float('inf')

    #
    ## HANDLING EACH GALAXY:
    #

    # Empty graph to fill
    result = Graph()

    # Iterate through system dict
    for curr_galaxy, data in system_dict.items():
        # Grab data members
        graph = data["graph"]
        arrival_teleport = data["arrival_teleport"]
        departure_teleport = data["departure_teleport"]
        departure_destinations = data["departure_destinations"]

        path, dist = graph.a_star(arrival_teleport, departure_teleport, Vertex.taxicab_distance)
        if path:
            for dest in departure_destinations:
                result.add_to_graph(curr_galaxy, dest, dist)

        # We've reached the start galaxy
        if curr_galaxy == start_galaxy:
            # Find the shortest path through the galaxy
            path, dist = graph.a_star(start_v_id, departure_teleport, Vertex.taxicab_distance)

            # If there's a valid path, add it to the graph
            if path:
                # Take care of every possibly destination
                for dest in departure_destinations:
                    result.add_to_graph(curr_galaxy + "start", dest, dist)


            if curr_galaxy == end_galaxy:
                # Find the shortest path through the galaxy
                path, dist = graph.a_star(start_v_id, end_v_id, Vertex.taxicab_distance)

                # If there's a valid path, add it to the graph
                if path:
                    result.add_to_graph(curr_galaxy + "start", curr_galaxy + "end", dist)

        # We've reached the end galaxy
        if curr_galaxy == end_galaxy:
            # Find the shortest path through the galaxy
            path, dist = graph.a_star(arrival_teleport, end_v_id, Vertex.taxicab_distance)

            # If there's a valid path, add it to the graph
            if path:
                result.add_to_graph(curr_galaxy, curr_galaxy + "end", dist)

    # Return the distance from the start to end from the result graph we built
    path, dist = result.a_star(start_galaxy + "start", end_galaxy + "end", Vertex.taxicab_distance)
    if path:
        return dist
    return float('inf')
